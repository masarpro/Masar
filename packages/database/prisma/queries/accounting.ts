...
export async function seedChartOfAccounts(
	...
	if (cashParentId && bankAccounts.length > 0) {
		for (let i = 0; i < bankAccounts.length; i++) {
			const bankCode = `111${(i + 1).toString().padStart(1, "0")}`;
...
export async function createJournalEntry(db: PrismaClient, data: {
	...
	if (periodClosed) {
		if (data.isAutoGenerated) {
			// Auto-generated entries: skip silently (don't break the financial operation)
			console.warn(`[AutoJournal] Skipped — period closed for date ${data.date.toISOString()}`);
			return null;
		}
	...
	return db.journalEntry.create({
		data: {
			...
			status,
			...(data.isAutoGenerated ? { postedAt: new Date() } : {}),
