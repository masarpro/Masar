...
export async function createJournalEntry(db: PrismaClient, data: {
	...
	createdById?: string;
	// Year-end closing entries (and reversals of them) legitimately post into the
	// already-closed fiscal year on 31 Dec — they must bypass the closed-period
	// guard, otherwise the whole year-end close silently no-ops (returns null).
	...
	// Generate entry number with type-specific prefix (atomic — uses its own SQL transaction)
	...
	// Wrap all DB checks + creation in a single transaction for atomicity.
	// When an existing transaction client is supplied (e.g. year-end closing),
	// run inside it so the caller's outer transaction stays atomic instead of
	// opening a nested independent transaction.
	const execute = async (tx: Prisma.TransactionClient) => {
		// Check for duplicate auto-generated entries. A transaction-scoped advisory
		// lock keyed by (org, referenceType, referenceId) serializes concurrent
		// creates for the SAME source document so the findFirst dedup below is
		// race-free — Prisma can't express the partial-unique index this needs
		// (WHERE status != 'REVERSED'), so this closes the TOCTOU window instead.
		if (data.isAutoGenerated && data.referenceType && data.referenceId) {
			await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`je:${data.organizationId}:${data.referenceType}:${data.referenceId}`}))`;
		...
		// Year-end closing/retained entries — and reversals of them — legitimately
		// post into the closed fiscal year; skip the closed-period guard for them.
		const isYearEndEntry =
			data.referenceType === "YEAR_END_CLOSING" ||
			data.referenceType === "YEAR_END_RETAINED" ||
			data.allowClosedPeriod === true;

		// Check if the entry date falls in a closed period
		const closedPeriod = isYearEndEntry
			? null
		...
		if (closedPeriod) {
			if (data.isAutoGenerated) {
				...
				if (data.createdById) {
					// Await inside the transaction so the skip is actually recorded —
					// a floating promise here fails with "transaction already closed"
					// and the only trace of a skipped entry would be lost.
					try {
						...
						console.error("[AutoJournal] Failed to record JOURNAL_ENTRY_SKIPPED:", auditErr);
					}
				}
				return null;
			}
...
export async function reverseJournalEntry(
	...
	// Year-end closing/retained entries live on 31 Dec inside the closed fiscal
	// year — reversing them (reopening the year) must be allowed. Every other
	// entry in a closed period stays protected.
	const isYearEndEntry =
		entry.referenceType === "YEAR_END_CLOSING" ||
		entry.referenceType === "YEAR_END_RETAINED";
	...
	// Create the reversal + post it + mark the original as reversed atomically.
	// A crash between these steps previously left the reversal POSTED while the
	// original stayed POSTED/unmarked, allowing it to be reversed twice. When the
	// caller supplies a transaction (e.g. year-end reversal), join it so the whole
	// multi-entry reversal is one atomic unit.
	const run = async (tx: Prisma.TransactionClient) => {
		const reversalEntry = await createJournalEntry(db, {
			...
			allowClosedPeriod: isYearEndEntry,
		...
		// Mark original as reversed — optimistic guard on status: only the first
		// concurrent reversal wins, a second matches zero rows and aborts instead
		// of double-reversing the same entry.
		const marked = await tx.journalEntry.updateMany({
			where: { id: entryId, status: "POSTED" },
			data: { isReversed: true, reversalId: reversalEntry.id, status: "REVERSED" },
		});
		if (marked.count === 0) {
			throw new Error("تم عكس هذا القيد بالفعل");
		}
...
export async function closePeriod(
	...
	if (options?.generateClosingEntry) {
		...
		// Post the closing entry AND flag the period closed atomically — otherwise a
		// crash after posting leaves a closing entry in an open period that still
		// accepts new entries (double-closing risk).
		if (lines.length > 0) {
...
export async function saveOpeningBalances(
	...
	return db.$transaction(async (tx) => {
		// Serialize concurrent opening-balance saves for this org (there is no
		// natural row to lock and no unique on OPENING_BALANCE): a transaction-
		// scoped advisory lock prevents two callers from both creating an entry.
		await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`opening-balance:${organizationId}`}))`;
		...
		// Don't silently rewrite an opening-balance entry that lives in a closed
		// period — deleting it would mutate closed-period totals with no trace.
		if (existing) {
			const periodClosed = await isPeriodClosed(db, organizationId, existing.date);
			if (periodClosed) {
				throw new Error(
					"لا يمكن تعديل الأرصدة الافتتاحية — قيدها في فترة محاسبية مغلقة",
				);
			}
		}
