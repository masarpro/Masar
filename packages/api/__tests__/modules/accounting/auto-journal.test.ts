...
describe("Auto-Journal Engine", () => {
	...
	// ─── 13. Owner Drawing ──────────────────────────────────────────────
	describe("onOwnerDrawing", () => {
		const drawingBase = {
			id: "draw-1",
			organizationId: "org-1",
			drawingNo: "OD-2026-0001",
			amount: D(25000),
			date: new Date("2026-04-10"),
			ownerName: "جودت",
			drawingsAccountId: "acc-owner-drawings",
			userId: "user-1",
		};

		it("should create DR owner drawings (34xx) / CR bank when bankAccountId given", async () => {
			setupBankLookup(db, "acc-bank-gl");

			const entryId = await onOwnerDrawing(db, {
				...drawingBase,
				bankAccountId: "bank-1",
			});

			const entry = getEntryData();
			expect(entry.referenceType).toBe("OWNER_DRAWING");
			expect(entry.referenceId).toBe("draw-1");
			expect(entry.referenceNo).toBe("OD-2026-0001");
			expect(entry.isAutoGenerated).toBe(true);

			const lines = entry.lines;
			expect(lines).toHaveLength(2);
			expect(lines[0].accountId).toBe("acc-owner-drawings"); // DR 34xx
			expect(Number(lines[0].debit)).toBe(25000);
			expect(lines[1].accountId).toBe("acc-bank-gl"); // CR bank
			expect(Number(lines[1].credit)).toBe(25000);
			expectBalancedLines(lines);
			expect(entryId).toBe("je-1");
		});

		it("should fall back to cash (1110) when no bankAccountId", async () => {
			setupAccountLookup(db, { "1110": "acc-cash" });

			await onOwnerDrawing(db, { ...drawingBase, bankAccountId: null });
		...
		it("should propagate projectId to both lines for project-level drawings", async () => {
			setupAccountLookup(db, { "1110": "acc-cash" });

			await onOwnerDrawing(db, {
				...drawingBase,
				bankAccountId: null,
				projectId: "proj-9",
			...
			const lines = getEntryLines();
			expect(lines[0].projectId).toBe("proj-9");
			expect(lines[1].projectId).toBe("proj-9");
		});

		it("should return null silently when bank account cannot be resolved", async () => {
			// لا bankAccountId ولا حساب 1110 → انسحاب صامت
			await expect(
				onOwnerDrawing(db, { ...drawingBase, bankAccountId: null }),
			).resolves.toBeNull();
			expect(mockCreateJournalEntry).not.toHaveBeenCalled();
		});
	});

	// ─── 14. Owner Drawing Cancelled ────────────────────────────────────
	describe("onOwnerDrawingCancelled", () => {
		it("should reverse the POSTED auto-generated entry", async () => {
			...
			expect(db.journalEntry.findFirst).toHaveBeenCalledWith(
				expect.objectContaining({
					where: expect.objectContaining({
						organizationId: "org-1",
		...
		it("should never throw even when the reversal fails", async () => {
			db.journalEntry.findFirst.mockRejectedValue(new Error("DB down"));

			await expect(
				onOwnerDrawingCancelled(db, {
					organizationId: "org-1",
					drawingId: "draw-err",
					userId: "user-1",
				}),
	...
	describe("onCapitalContribution", () => {
		const contributionBase = {
			id: "cc-1",
			organizationId: "org-1",
			contributionNo: "CC-2026-0001",
			amount: D(100000),
			date: new Date("2026-02-01"),
			ownerName: "جودت",
			type: "INITIAL",
		...
		it("should create DR bank / CR capital (3100) when bankAccountId given", async () => {
			setupAccountLookup(db, { "3100": "acc-capital" });
			setupBankLookup(db, "acc-bank-gl");

			const entryId = await onCapitalContribution(db, {
				...contributionBase,
				bankAccountId: "bank-1",
			});

			const entry = getEntryData();
			expect(entry.referenceType).toBe("CAPITAL_CONTRIBUTION");
			expect(entry.referenceId).toBe("cc-1");
			expect(entry.referenceNo).toBe("CC-2026-0001");

			const lines = entry.lines;
			expect(lines).toHaveLength(2);
			expect(lines[0].accountId).toBe("acc-bank-gl"); // DR bank (capital in)
			expect(Number(lines[0].debit)).toBe(100000);
			expect(lines[1].accountId).toBe("acc-capital"); // CR 3100
			expect(Number(lines[1].credit)).toBe(100000);
			expectBalancedLines(lines);
			expect(entryId).toBe("je-1");
		});

		it("should fall back to cash (1110) when no bankAccountId", async () => {
			setupAccountLookup(db, { "1110": "acc-cash", "3100": "acc-capital" });

			await onCapitalContribution(db, {
				...contributionBase,
				bankAccountId: null,
			});
			...
			const lines = getEntryLines();
			expect(lines[0].accountId).toBe("acc-cash");
			expect(lines[1].accountId).toBe("acc-capital");
			expectBalancedLines(lines);
		});

		it("should return null silently when capital account (3100) is missing", async () => {
			setupAccountLookup(db, { "1110": "acc-cash" }); // لا يوجد 3100

			await expect(
				onCapitalContribution(db, { ...contributionBase, bankAccountId: null }),
			).resolves.toBeNull();
			expect(mockCreateJournalEntry).not.toHaveBeenCalled();
		});
	});
