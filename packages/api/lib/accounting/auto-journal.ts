...
//
// Complete Operations Map (30 operations):
//
// الفواتير (Invoices):
...
//
// سحوبات الشركاء (Owner Drawings):
//   #30 سحب شريك            → onOwnerDrawing             → DR 34xx / CR بنك
//   #31 إلغاء سحب شريك      → onOwnerDrawingCancelled    → عكس #30
//
// مساهمات رأس المال (Capital Contributions):
//   #32 مساهمة رأس مال       → onCapitalContribution      → DR بنك / CR 3100
//   #33 إلغاء مساهمة         → reverseAutoJournalEntry    → عكس #32
//
...
// ========================================
// 13. Owner Drawing Approved → DR: Owner Drawings (34xx) / CR: Bank
// ========================================
export async function onOwnerDrawing(db: PrismaClient, drawing: {
	id: string;
	organizationId: string;
	drawingNo: string;
	amount: Prisma.Decimal;
	date: Date;
	ownerName: string;
	drawingsAccountId: string; // the owner's 34xx chart account id
	...
	try {
		if (!(await ensureChartExists(db, drawing.organizationId))) return null;

		// Credit: Bank/Cash source
		const bankAccId = drawing.bankAccountId
		...
		const desc = drawing.projectId && drawing.projectName
			? `سحب مالك — ${drawing.ownerName} — مشروع ${drawing.projectName} — ${drawing.drawingNo} | Owner drawing — ${drawing.ownerName} — Project ${drawing.projectName} — ${drawing.drawingNo}`
			: `سحب مالك — ${drawing.ownerName} — ${drawing.drawingNo} | Owner drawing — ${drawing.ownerName} — ${drawing.drawingNo}`;
		...
		const entry = await createJournalEntry(db, {
			...
			isAutoGenerated: true,
			createdById: drawing.userId,
			lines: [
				{
					accountId: drawing.drawingsAccountId,
					debit: drawing.amount,
					credit: ZERO,
					projectId: drawing.projectId ?? undefined,
				},
				{
					accountId: bankAccId,
					debit: ZERO,
		...
		console.error("[AutoJournal] onOwnerDrawing failed:", error);
		return null;
	}
}

// ========================================
// 14. Owner Drawing Cancelled → Reverse the auto-generated entry
// ========================================
export async function onOwnerDrawingCancelled(db: PrismaClient, data: {
	organizationId: string;
	drawingId: string;
	userId: string;
}): Promise<void> {
	try {
		await reverseAutoJournalEntry(db, {
			organizationId: data.organizationId,
			referenceType: "OWNER_DRAWING",
			referenceId: data.drawingId,
			userId: data.userId,
		});
	} catch (error) {
		console.error("[AutoJournal] onOwnerDrawingCancelled failed:", error);
	}
}

// ========================================
// 15. Capital Contribution → DR: Bank/Cash / CR: Capital (3100)
// ========================================
export async function onCapitalContribution(db: PrismaClient, data: {
	id: string;
	organizationId: string;
	...
	try {
		if (!(await ensureChartExists(db, data.organizationId))) return null;

		// Debit: Bank/Cash source (capital comes IN)
		const bankAccId = data.bankAccountId
			? await getBankChartAccountId(db, data.organizationId, data.bankAccountId)
			: await getAccountByCode(db, data.organizationId, "1110");
		...
		const entry = await createJournalEntry(db, {
			...
			referenceId: data.id,
			referenceNo: data.contributionNo,
			isAutoGenerated: true,
		...
		console.error("[AutoJournal] onCapitalContribution failed:", error);
		return null;
	}
}

// ========================================
// 16. Final Handover Completed → Retention Release
