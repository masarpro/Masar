...
> **تحديث (v8 / EH-8):** الحظر أعلاه يخص مخططات **الإخراج** (output) حيث `null` يكسر التسلسل. أما في مخططات **الإدخال** (input) فالنمطان `.nullish()`/`.trim().optional()` يعملان فعلياً عبر الدوال المركزية `nullishTrimmed`/`optionalTrimmed` في `packages/api/lib/validation-constants.ts` (مستخدمة في مسارات إنتاجية كثيرة). استخدم هذه الدوال المركزية للحقول الاختيارية بدل صياغة يدوية.

> **حماية عامة (v8 / EH-4):** أُضيف middleware في `packages/api/orpc/procedures.ts` على `publicProcedure` يحوّل أي `throw new Error("رسالة عربية")` إلى `ORPCError("BAD_REQUEST")` تلقائياً — فالرسائل العربية الموجهة للمستخدم من طبقة الاستعلام تصل للعميل بدل 500 مبهم. مع ذلك، الأفضل رمي `ORPCError` صراحةً في الإجراءات الجديدة.
...
**ملاحظة (2026-07-20):** أُلغي نظام الاعتماد/إعادة الفتح بالكامل — **كل المراحل حية دائماً** (لا Lock gates ولا StageApprovalButton). زر "تحويل لعرض سعر" متاح دائماً للأنواع المسموحة. تسعير التكلفة والتسعير مدموجان في **لوحة واحدة** (`CostingPageContentV2` بتبويبات: المواد | المصنعيات | مصاريف غير مباشرة | ملخص التكلفة | التسعير والأرباح). المصاريف غير المباشرة (سلك ومسمار من BOQ، إشراف هندسي/ميداني، مصاريف تشغيل) تُخزن في `laborBreakdown.indirectCosts` وتُحسب على السيرفر في `packages/api/modules/quantities/lib/indirect-costs.ts` وتدخل التكلفة قبل الربح وقبل VAT. حالة الدراسة تُشتق الآن من تقدّم البيانات (بنود/عرض سعر) في `deriveCostStudyStatus`. إجراءات approve/reopen في الـ API ما زالت موجودة لكنها غير مستخدمة من الواجهة.
...
**نقاط الاستدعاء:** الداشبورد (`sections/QuickActionsGrid`)، قائمة المصروفات (`ExpensesList`)، سياق المشروع (`ExpensesTable` + `QuickActions` overview + `BankDetail`). *(ملاحظة v8/FE-05: حُذفت مكوّنات الداشبورد المالي القديمة `finance/components/dashboard/{ActionCards,QuickActionsGrid,…}` ككود ميت بعد إعادة تصميم Botly — لم تعد نقاط استدعاء.)*
