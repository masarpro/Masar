...
export function InvoicePreview({
	...
	// ?print=1 (من قائمة الفواتير): أطلق الطباعة تلقائياً فور اكتمال التحميل
	const searchParams = useSearchParams();
	const autoPrintFired = useRef(false);
	const shouldAutoPrint = searchParams.get("print") === "1";
	useEffect(() => {
		if (shouldAutoPrint && !isLoading && invoice && !autoPrintFired.current) {
			autoPrintFired.current = true;
			const timer = setTimeout(() => printDocument(), 500);
	...
	return (
		<div className="-mx-4 -mt-2 px-4 pt-0 pb-24 sm:-mx-6 sm:px-6 min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-50 via-slate-100/40 to-slate-50 dark:from-slate-950 dark:via-slate-900/40 dark:to-slate-950">
			...
			{/* ─── A4 Invoice Document ─────────────────────────── */}
			<Card id="invoice-print-area" className="rounded-2xl max-w-[210mm] min-h-[297mm] mx-auto overflow-hidden print:shadow-none print:rounded-none print:border-none print:min-h-0 print:max-w-none">
