...
export function QuotationForm({
	...
	const createMutation = useMutation({
		mutationFn: async () => {
			...
			return orpcClient.finance.quotations.create({
				organizationId,
				clientId,
				clientName,
				clientCompany,
		...
		onSuccess: (data) => {
			toast.success(t("finance.quotations.createSuccess"));
			router.push(`${basePath}/${data.id}`);
		},
		onError: (error: any) => {
			toast.error(error.message || t("finance.quotations.createError"));
		},
	});
	...
	const updateMutation = useMutation({
		mutationFn: async () => {
			...
			return orpcClient.finance.quotations.update(updateData);
		},
		onError: (error: any) => {
			console.error("Update mutation error:", error);
			toast.error(error.message || t("finance.quotations.updateError"));
		},
	});
	...
	const updateItemsMutation = useMutation({
		mutationFn: async () => {
			...
			return orpcClient.finance.quotations.updateItems(itemsData);
		},
		onError: (error: any) => {
			toast.error(error.message || t("finance.quotations.itemsUpdateError"));
		},
	});
	...
	const statusMutation = useMutation({
		mutationFn: async (status: "DRAFT" | "SENT" | "ACCEPTED" | "REJECTED" | "EXPIRED") => {
			await orpcClient.finance.quotations.updateStatus({
				organizationId,
				id: quotationId!,
				status,
			});
		...
		onSuccess: (_, status) => {
			toast.success(t(`finance.quotations.status.${status.toLowerCase()}Success`));
			queryClient.invalidateQueries({
				queryKey: ["finance", "quotations"],
			});
		},
		onError: (error: any) => {
			toast.error(error.message || t("finance.quotations.statusUpdateError"));
		},
	});
	...
	const convertMutation = useMutation({
		mutationFn: async () => {
			return orpcClient.finance.quotations.convertToInvoice({
				organizationId,
				id: quotationId!,
			});
		},
		onSuccess: (data) => {
			toast.success(t("finance.quotations.convertSuccess"));
			router.push(`/app/${organizationSlug}/finance/invoices/${data.id}`);
		},
		onError: (error: any) => {
			toast.error(error.message || t("finance.quotations.convertError"));
		},
	});
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!clientName.trim()) {
			toast.error(t("finance.quotations.errors.clientRequired"));
			return;
		}

		const validItems = items.filter((item) => item.description.trim());
		if (validItems.length === 0) {
			toast.error(t("finance.quotations.errors.itemsRequired"));
			return;
		}
		...
		if (mode === "create") {
			...
			try {
				...
				toast.success(t("finance.quotations.updateSuccess"));
	...
	const columnLabels: Record<ColumnKey, string> = {
		...
		discount: t("finance.quotations.discountPercent"),
		tax: t("finance.quotations.vatPercent"),
		...
		<form onSubmit={handleSubmit} className="pb-28 lg:pb-6 space-y-4">
							...
							<DropdownMenuContent align="end" className="rounded-xl">
								{existingQuotation.status === "DRAFT" && (
									...
									>
										...
										{t("finance.quotations.actions.send")}
									</DropdownMenuItem>
								)}
								{(existingQuotation.status === "SENT" || existingQuotation.status === "VIEWED") && (
									<>
										...
										>
											...
											{t("finance.quotations.actions.accept")}
										</DropdownMenuItem>
										<DropdownMenuItem
										...
										>
											...
											{t("finance.quotations.actions.reject")}
										</DropdownMenuItem>
									</>
								)}
								{existingQuotation.status === "ACCEPTED" && (
									...
									>
										...
										{t("finance.quotations.actions.convertToInvoice")}
									</DropdownMenuItem>
								)}
								<DropdownMenuSeparator />
								<DropdownMenuItem
								...
								>
									{t("finance.quotations.actions.revertToDraft")}
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
					)}
			...
			{mode === "edit" && !isEditable && (
				<div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
					<p className="text-amber-800 dark:text-amber-200 text-sm">
						{t("finance.quotations.notEditable")}
					</p>
				</div>
			)}
			...
			<div className="grid gap-4 lg:grid-cols-6">
				...
				<div className="lg:col-span-1 rounded-2xl border bg-card p-3 space-y-2">
					<div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
						...
						{t("finance.quotations.documentSettings")}
					</div>
					...
					<div className="grid gap-2">
						...
						<Select value={salesRepId} onValueChange={setSalesRepId} disabled={!isEditable}>
							<SelectTrigger className="rounded-lg h-9 text-xs">
								<SelectValue placeholder={t("finance.quotations.salesRep")} />
							</SelectTrigger>
					...
					>
						...
						<span className="text-sm font-medium">
							{isSaving
								...
								: mode === "create"
									? t("common.save")
									: t("finance.quotations.saveChanges")
							}
						</span>
					</Button>
			...
			<div className="rounded-2xl border bg-card overflow-hidden">
				...
				<div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
					<div className="flex items-center gap-2">
						<span className="text-sm font-semibold">{t("finance.quotations.items")}</span>
			...
			<div className="grid gap-4 lg:grid-cols-5">
					...
					<Collapsible open={termsOpen} onOpenChange={setTermsOpen}>
						...
						<CollapsibleContent className="pt-3 space-y-3">
							<div className="grid gap-3 sm:grid-cols-2">
								<div>
									<Label className="text-xs text-muted-foreground">{t("finance.quotations.paymentTerms")}</Label>
									<Textarea
										value={paymentTerms}
										onChange={(e) => setPaymentTerms(e.target.value)}
										placeholder={t("finance.quotations.paymentTermsPlaceholder")}
										rows={2}
										disabled={!isEditable}
								...
								<div>
									<Label className="text-xs text-muted-foreground">{t("finance.quotations.deliveryTerms")}</Label>
									<Textarea
										value={deliveryTerms}
										onChange={(e) => setDeliveryTerms(e.target.value)}
										placeholder={t("finance.quotations.deliveryTermsPlaceholder")}
										rows={2}
										disabled={!isEditable}
										className="rounded-lg mt-1 text-sm"
									/>
								...
								<div>
									<Label className="text-xs text-muted-foreground">{t("finance.quotations.warrantyTerms")}</Label>
									<Textarea
										value={warrantyTerms}
										onChange={(e) => setWarrantyTerms(e.target.value)}
										placeholder={t("finance.quotations.warrantyTermsPlaceholder")}
										rows={2}
										disabled={!isEditable}
										className="rounded-lg mt-1 text-sm"
									/>
								...
								<div>
									<Label className="text-xs text-muted-foreground">{t("finance.quotations.notes")}</Label>
									<Textarea
										value={notes}
										onChange={(e) => setNotes(e.target.value)}
										placeholder={t("finance.quotations.notesPlaceholder")}
										rows={2}
										disabled={!isEditable}
										className="rounded-lg mt-1 text-sm"
									/>
				...
				<div className="lg:col-span-2 rounded-2xl border bg-card p-4 space-y-4">
					...
					{includeDiscount && globalDiscountValue > 0 && (
						<div className="flex items-center justify-between text-sm text-orange-600">
							<span>
								{discountType === "percent" ? `${globalDiscountPercent}%` : t("finance.quotations.fixedAmount")}
							</span>
				...
				<div className="flex items-center justify-between gap-4">
					...
					{isEditable && (
						<Button type="submit" size="lg" className="rounded-xl px-8" disabled={isSaving}>
							...
							{isSaving ? t("common.saving") : (mode === "create" ? t("common.save") : t("finance.quotations.saveChanges"))}
						</Button>
					)}
				</div>
			</div>
