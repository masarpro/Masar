...
export function VIForm({ organizationId, organizationSlug }: VIFormProps) {
	const t = useTranslations();
	const router = useRouter();
	const queryClient = useQueryClient();
	const basePath = `/app/${organizationSlug}/procurement/invoices`;
	...
	const updateItem = (index: number, field: keyof VIItem, value: string | number) => {
		const updated = [...items];
	...
	const mutation = useMutation({
		mutationFn: async () => {
			return orpcClient.procurement.vendorInvoices.create({
				organizationId,
				vendorId,
		...
		onSuccess: () => {
			toast.success(t("procurement.invoiceSaved"));
	...
	return (
		<div className="max-w-3xl space-y-6">
			<Card className="rounded-2xl">
				<CardHeader><CardTitle>{t("procurement.invoice")}</CardTitle></CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
						<div className="space-y-2">
							<Label>{t("procurement.vendorInvoiceNo")}</Label>
							<Input value={vendorInvoiceNo} onChange={(e) => setVendorInvoiceNo(e.target.value)} className="rounded-xl" dir="ltr" />
						</div>
						<div className="space-y-2">
							<Label>{t("procurement.invoiceDate")} *</Label>
							<Input type="date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} className="rounded-xl" dir="ltr" />
						</div>
						<div className="space-y-2">
							<Label>{t("procurement.dueDate")}</Label>
							<Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="rounded-xl" dir="ltr" />
						</div>
					</div>
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						<div className="space-y-2">
