...
const PROJECT_TYPES = [
	{ value: "residential", labelKey: "quantities.projectTypes.residential", color: "bg-sky-500" },
	{ value: "commercial", labelKey: "quantities.projectTypes.commercial", color: "bg-violet-500" },
	{ value: "industrial", labelKey: "quantities.projectTypes.industrial", color: "bg-orange-500" },
	{ value: "warehouse", labelKey: "quantities.projectTypes.warehouse", color: "bg-slate-500" },
	{ value: "mixed", labelKey: "quantities.projectTypes.mixed", color: "bg-teal-500" },
];

const FINISHING_LEVELS = [
	{ value: "economic", labelKey: "quantities.finishingLevels.economic" },
	{ value: "medium", labelKey: "quantities.finishingLevels.medium" },
	{ value: "luxury", labelKey: "quantities.finishingLevels.luxury" },
	{ value: "super_luxury", labelKey: "quantities.finishingLevels.superLuxury" },
];
...
export function CreateCostStudyForm({
	...
	const createMutation = useMutation(
		orpc.quantities.create.mutationOptions({
			onSuccess: (data) => {
				toast.success(t("quantities.createSuccess"));
				router.push(`/app/${organizationSlug}/quantities/${data.id}`);
			},
			onError: () => {
				toast.error(t("quantities.createError"));
			},
		}),
	);
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (isNaN(landArea) || landArea <= 0) {
			toast.error(t("quantities.validation.landArea"));
			return;
		}

		if (isNaN(buildingArea) || buildingArea <= 0) {
			toast.error(t("quantities.validation.buildingArea"));
			return;
		}

		if (isNaN(numberOfFloors) || numberOfFloors <= 0) {
			toast.error(t("quantities.validation.numberOfFloors"));
			return;
		}
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-6">
			...
			<div className="flex items-center gap-4">
				...
				<div>
					<h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{t("quantities.newStudy")}</h1>
					<p className="text-slate-500 dark:text-slate-400">{t("quantities.subtitle")}</p>
				</div>
			</div>
				...
				<div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden">
					<div className="p-5 border-b border-slate-100 dark:border-slate-800">
						<div className="flex items-center gap-3">
							...
							<h2 className="text-lg font-medium text-slate-900 dark:text-slate-100">{t("quantities.form.basicInfo")}</h2>
						</div>
					</div>
					...
					<div className="p-5 space-y-5">
						<div className="space-y-2">
							...
							<Input
								...
								placeholder={t("quantities.form.namePlaceholder")}
						...
						<div className="space-y-2">
							<Label htmlFor="customerName" className="text-sm font-medium text-slate-700 dark:text-slate-300">
								{t("quantities.form.customerName")}
							</Label>
							<Input
								id="customerName"
								...
								placeholder={t("quantities.form.customerNamePlaceholder")}
								className="rounded-xl bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700"
							/>
						</div>
						...
						<div className="space-y-2">
							<Label htmlFor="finishingLevel" className="text-sm font-medium text-slate-700 dark:text-slate-300">
								{t("quantities.form.finishingLevel")}
							</Label>
							<Select
				...
				<div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden">
					<div className="p-5 border-b border-slate-100 dark:border-slate-800">
						<div className="flex items-center gap-3">
							...
							<h2 className="text-lg font-medium text-slate-900 dark:text-slate-100">{t("quantities.form.dimensions")}</h2>
						</div>
					</div>
					...
					<div className="p-5 space-y-5">
						...
						<div className="space-y-2">
							<Label htmlFor="buildingArea" className="text-sm font-medium text-slate-700 dark:text-slate-300">
								{t("quantities.form.buildingArea")}
							</Label>
							<div className="relative">
								<Input
									id="buildingArea"
						...
						<div className="space-y-2">
							<Label htmlFor="numberOfFloors" className="text-sm font-medium text-slate-700 dark:text-slate-300">
								{t("quantities.form.numberOfFloors")}
							</Label>
							<Input
								id="numberOfFloors"
								type="number"
