...
export function StructuralAccordion({
	...
	const sections = [
		{
			id: "plainConcrete",
			title: t("quantities.structural.plainConcrete"),
			icon: "🧱",
			component: PlainConcreteSection,
		},
		{
			id: "foundations",
			title: t("quantities.structural.reinforcedFoundations"),
			icon: "🏗️",
			component: FoundationsSection,
		},
		{
			id: "beams",
			title: t("quantities.structural.beams"),
			icon: "📏",
			component: BeamsSection,
		},
		{
			id: "columns",
			title: t("quantities.structural.columns"),
			icon: "🏛️",
			component: ColumnsSection,
		},
		{
			id: "slabs",
			title: t("quantities.structural.slabs"),
			icon: "⬛",
			component: SlabsSection,
		},
		{
			id: "blocks",
			title: t("quantities.structural.blocks"),
			icon: "🧱",
			component: BlocksSection,
		},
		{
			id: "stairs",
			title: t("quantities.structural.stairs"),
			icon: "🪜",
								...
								<div className="flex items-center gap-2">
									{isSaved ? (
										<Badge variant="default" className="bg-green-600">
											...
											{t("quantities.messages.saved")}
										</Badge>
									) : hasItems ? (
										<Badge variant="secondary">
											{sectionItems.length} {t("quantities.items")}
										</Badge>
									) : (
										<Badge variant="outline" className="text-muted-foreground">
											<AlertCircle className="h-3 w-3 ml-1" />
											{t("quantities.structural.noItems")}
										</Badge>
									)}
								</div>
							</div>
