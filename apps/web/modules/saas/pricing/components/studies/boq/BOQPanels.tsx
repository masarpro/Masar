...
import { useBOQTables, useBOQTablesOptional } from "./BOQTablesContext";

// ═══════════════════════════════════════════════════════════════
// لوحات جداول الكميات
// ───────────────────────────────────────────────────────────────
// أربع لوحات مستقلة تقرأ من BOQTablesContext — تبقى مركّبة معاً
// فالتنقل بينها فوري، والفلاتر والتحديد مشتركة بينها كلها.
// ═══════════════════════════════════════════════════════════════

/** ملخص الكميات */
export function BOQSummaryPanel() {
	const {
		summary,
		expandedSections,
		expandedCutting,
	...
	if (items.length === 0) return <EmptyTable kind="summary" />;
...
/** طلبية المصنع */
export function BOQFactoryOrderPanel() {
	const { summary, unscheduledSteelWeight } = useBOQTables();
	return (
		<FactoryOrderTab
...
/** تفاصيل التفصيل */
export function BOQCuttingPanel() {
	const { summary, unscheduledSteelWeight } = useBOQTables();
	return (
		<CuttingWorkshopTab
...
/** التكلفة والتسعير (تقرير) */
export function BOQCostReportPanel() {
	const { costReport, costReportLoading, syncMaterials, isSyncingMaterials } =
		useBOQTables();

	if (costReportLoading || !costReport) {
		return (
...
function EmptyTable({ kind }: { kind: "summary" | "factory" | "cutting" }) {
	const t = useTranslations("pricing.studies");
	const ctx = useBOQTablesOptional();
	const Icon =
		kind === "factory" ? Factory : kind === "cutting" ? Scissors : Layers;
	const title =
		kind === "factory"
			? t("boq.noFactoryData")
			: kind === "cutting"
				? t("boq.noCuttingData")
				: t("boq.noSelectedItems");

	return (
		<Card className="p-10 text-center text-muted-foreground">
			<Icon className="mx-auto mb-3 h-12 w-12 opacity-25" />
			<p className="font-medium text-foreground/70">{title}</p>
			<p className="mt-1 text-xs">
				{ctx?.hasActiveFilters
					? t("boq.tryClearingFilters")
					: t("boq.addItemsWithRebarFirst")}
			</p>
			{ctx?.hasActiveFilters && (
				<button
					type="button"
					onClick={ctx.resetFilters}
					className="mt-3 text-primary text-xs underline-offset-4 hover:underline"
				>
					{t("boq.clearFilters")}
				</button>
			)}
...
/** صح على مجموعة بنود — نصف محدد حين يكون بعضها فقط */
function GroupCheckbox({ ids, label }: { ids: string[]; label: string }) {
	const { isSelected, setSelection } = useBOQTables();
	const selected = ids.filter(isSelected).length;
	const all = selected === ids.length && ids.length > 0;
	...
	return (
		<Checkbox
			...
			onCheckedChange={(value) =>
				setSelection(ids, value !== false && !some)
			}
...
function ItemCheckbox({ id, label }: { id: string; label: string }) {
	const { isSelected, toggleItem } = useBOQTables();
	return (
		<Checkbox
...
function FloorMaterialView({
	...
	const totalConcrete = concreteRows.reduce(
		(s, d) => s + d.item.concreteVolume,
		0,
	);
	...
	const sharedConcrete = sharedRows.reduce(
		(s, d) => s + d.item.concreteVolume,
		0,
	);
		...
		<div className="space-y-4">
			...
			{sharedRows.length > 0 && (
				<p className="text-xs text-muted-foreground">
					يشمل عناصر مشتركة بين الأدوار (
					{formatNumber(sharedConcrete)} م³ خرسانة،{" "}
					{formatNumber(sharedSteel)} كجم حديد)
				</p>
			)}
								...
								<TableHeader>
									<TableRow className="bg-muted/30">
										...
										<TableHead className="text-start text-xs">
											{t("structural.boq.element")}
										</TableHead>
										<TableHead className="text-start text-xs">
											{t("structural.boq.category")}
										</TableHead>
										<TableHead className="text-start text-xs">
											{t("structural.boq.quantity")}
										</TableHead>
										<TableHead className="text-start text-xs">
											{valueLabel}
										</TableHead>
...
function SectionCard({
	...
	return (
		<Card className={`border-s-4 ${borderColor} overflow-hidden`}>
			<Collapsible open={isExpanded} onOpenChange={onToggle}>
				{/* الصح خارج زر الطي — زر داخل زر HTML غير صالح ويكسر الترطيب */}
				<div className="flex items-center gap-3 ps-4 hover:bg-muted/30">
					<GroupCheckbox
						ids={section.subGroups.flatMap((g) =>
							g.items.map((d) => d.item.id),
						)}
						label={section.label}
					/>
					<CollapsibleTrigger asChild>
						<button
							type="button"
							className="flex flex-1 items-center justify-between px-1 py-3 pe-4 transition-colors"
						>
							<div className="flex items-center gap-3">
								{isExpanded ? (
									<ChevronDown className="h-4 w-4 text-muted-foreground" />
							...
							<div className="flex items-center gap-4 text-sm">
								...
								{section.totalBlocks > 0 && (
									<span className="text-success font-medium">
										{formatNumber(section.totalBlocks)}{" "}
										{t(
											"structural.otherStructural.results.blockUnit",
										)}
									</span>
								)}
							</div>
						...
						{section.extras && (
							<div className="flex flex-wrap gap-2 pt-1">
								{section.extras.plainConcrete > 0 && (
									...
									>
										{t(
											"structural.otherStructural.results.concretePlain",
										)}
										:{" "}
										{formatNumber(
											section.extras.plainConcrete,
										)}{" "}
										م³
									</Badge>
								)}
								{section.extras.grcWeight > 0 && (
									...
									>
										{t(
											"structural.otherStructural.results.grcWeight",
										)}
										:{" "}
										{formatNumber(section.extras.grcWeight)}{" "}
										كجم
									</Badge>
								)}
								{section.extras.waterproofingArea > 0 && (
									...
									>
										{t(
											"structural.otherStructural.results.waterproofing",
										)}
										:{" "}
										{formatNumber(
											section.extras.waterproofingArea,
										)}{" "}
										م²
									</Badge>
								)}
								{section.extras.excavationVolume > 0 && (
									...
									>
										{t(
											"structural.otherStructural.results.excavation",
										)}
										:{" "}
										{formatNumber(
											section.extras.excavationVolume,
										)}{" "}
										م³
									</Badge>
								)}
								{section.extras.mortarVolume > 0 && (
									...
									>
										{t(
											"structural.otherStructural.results.mortar",
										)}
										:{" "}
										{formatNumber(
											section.extras.mortarVolume,
										)}{" "}
										م³
									</Badge>
								)}
		...
		<div className="space-y-2 pt-2">
			{showGroupHeader && (
				<div className="flex items-center justify-between">
					<h5 className="flex items-center gap-2 font-semibold text-muted-foreground text-sm">
						<GroupCheckbox
							ids={group.items.map((d) => d.item.id)}
							label={group.label}
						/>
						{group.label}
					</h5>
					<div className="flex gap-3 text-xs text-muted-foreground">
						{group.concrete > 0 && (
							<span>
								{t("boq.concrete")}:{" "}
								{formatNumber(group.concrete)} م³
							</span>
						)}
						{group.rebar > 0 && (
							<span>
								{t("boq.steel")}: {formatNumber(group.rebar)}{" "}
								كجم
							</span>
						)}
						{group.blocks > 0 && (
							<span>
								{t("boq.blocks")}: {formatNumber(group.blocks)}
							</span>
						)}
					</div>
				</div>
				...
				<Table>
					<TableHeader>
						<TableRow className="bg-muted/30">
							...
							<TableHead className="w-8 text-start text-xs"></TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
							...
							return (
								<Fragment key={detail.item.id}>
									<TableRow>
										...
										<TableCell className="text-sm text-chart-4">
											{formatNumber(
												detail.item.concreteVolume,
											)}
										</TableCell>
										<TableCell className="text-sm text-chart-1">
											{formatNumber(
												detail.item.steelWeight,
											)}
										</TableCell>
										{category === "blocks" && (
				...
				<table className="w-full text-xs">
					<thead>
						<tr className="border-b text-muted-foreground">
							...
							<th className="text-start py-1 px-2">
								{t("structural.diameter")}
							</th>
							<th className="text-start py-1 px-2">
								{t("boq.pieceLength")}
							</th>
							<th className="text-start py-1 px-2">
								{t("structural.quantity")}
							</th>
							<th className="text-start py-1 px-2">
								{t("boq.factoryBars")}
							</th>
					...
					<tbody>
						{details.map((d, i) => (
							<tr key={i} className="border-b border-muted">
								...
								<td className="py-1 px-2">
									{formatNumber(d.grossWeight)} كجم
								</td>
							</tr>
						))}
					</tbody>
...
function GrandTotalCards({
	...
	const cards = [
		...
		},
		{
			...
			icon: Columns3,
			value:
				totals.rebar > 0 ? formatNumber(totals.rebar / 1000, 2) : "—",
			unit: "طن",
			subValue:
				totals.rebar > 0
...
function FactoryOrderTab({
	...
	const visible = factoryOrder.filter((e) => isDiameterSelected(e.diameter));
	const totalBars = visible.reduce((s, e) => s + e.count, 0);
	// الحديد المحسوب بالنِسَب يدخل الإجمالي — هو حديد يُشترى فعلاً
	const includeUnscheduled =
		selectedDiameters === null || selectedDiameters.size > 0;
	const unscheduled = includeUnscheduled ? unscheduledSteelWeight : 0;
	const totalWeight = visible.reduce((s, e) => s + e.weight, 0) + unscheduled;

	const allChecked = selectedDiameters === null;

	if (factoryOrder.length === 0 && unscheduledSteelWeight <= 0) {
		return <EmptyTable kind="factory" />;
	}
			...
			{!allChecked && (
				<div className="flex items-center gap-2 text-xs">
					<Badge variant="secondary">
						{t("boq.selectedDiameters", {
							count: selectedDiameters?.size ?? 0,
						})}
					</Badge>
					<button
						type="button"
						onClick={resetDiameters}
						className="text-primary underline-offset-4 hover:underline"
					>
						{t("boq.showAllDiameters")}
					</button>
				</div>
			)}
			...
			<Card className="overflow-hidden">
				<div className="overflow-x-auto">
					<Table>
						...
						<TableBody>
							{factoryOrder.map((entry) => {
								const checked = isDiameterSelected(
									entry.diameter,
								);
								return (
									<TableRow
										key={`${entry.diameter}-${entry.stockLength}`}
										className={
											checked ? undefined : "opacity-40"
										}
									>
										<TableCell>
											<Checkbox
							...
							{unscheduled > 0 && (
								<TableRow>
									...
									<TableCell>
										{formatNumber(unscheduled)}
									</TableCell>
									<TableCell className="font-bold text-chart-1">
										{formatNumber(unscheduled / 1000, 3)}
									</TableCell>
								</TableRow>
							)}
...
function CuttingWorkshopTab({
	...
	if (cuttingDetails.length === 0) {
		return <EmptyTable kind="cutting" />;
	}
	...
	return (
		<div className="space-y-4">
			...
			{sortedDiameters.map((diameter) => {
				...
				const groupStocks = group.reduce(
					(s, d) => s + d.stocksNeeded,
					0,
				);
				const checked = isDiameterSelected(diameter);

				return (
					<Card
						key={diameter}
						className={checked ? undefined : "opacity-50"}
					>
						<div className="flex items-center justify-between border-b bg-muted/30 px-4 py-2">
							<div className="flex items-center gap-2">
								<Checkbox
									...
									aria-label={`Ø${diameter}`}
								/>
								<Badge className="bg-chart-1/15 text-chart-1">
									Ø{diameter} مم
								</Badge>
								<span className="text-xs text-muted-foreground">
									{t("boq.cutOperationsCount", {
										count: group.length,
									})}
								</span>
							</div>
	...
	return (
		...
		>
			<table className="w-full caption-bottom text-sm">
				<TableHeader
					className={
						isVirtualized ? "sticky top-0 z-10 bg-background" : ""
					}
				>
					<TableRow>
						<TableHead className="text-start text-xs">
				...
				<TableBody>
					...
					{(isVirtualized
						? virtualItems.map(
								(vi: { index: number }) => group[vi.index],
							)
						: group
					).map((d: CuttingDetailRow, i: number) => (
						<TableRow
							key={isVirtualized ? virtualItems[i].index : i}
						>
							<TableCell className="text-sm">
								{d.element}
							</TableCell>
							...
							<TableCell className="text-sm">
								...
								{(d.reusedPieces ?? 0) > 0 && (
									...
									>
										(
										{t("boq.reusedFromRemnants", {
											count: d.reusedPieces ?? 0,
										})}
										)
									</span>
								)}
