						...
						<TableBody>
							{items.map((item) => (
								<TableRow key={item.id}>
									...
									<TableCell>
										{formatNumber((item.dimensions?.length || 0) * (item.dimensions?.width || 0))} {t("pricing.studies.units.m2")}
									</TableCell>
									<TableCell>
									...
									{showFormwork && calculations.formworkArea > 0 && (
										<div className="flex items-center gap-2 text-blue-600">
											...
											<span className="font-bold">
												{formatNumber(calculations.formworkArea)} {t("pricing.studies.units.m2")}
											</span>
										</div>
									)}
								</div>
					...
					<div className="grid grid-cols-3 gap-4 text-sm">
						<div>
							...
							<p className="font-bold">
								{formatNumber(items.reduce((sum, i) => sum + (i.dimensions?.length || 0) * (i.dimensions?.width || 0) * i.quantity, 0))} {t("pricing.studies.units.m2")}
							</p>
						</div>
						<div>
							<span className="text-muted-foreground">{t("pricing.studies.summary.totalConcrete")}:</span>
							<p className="font-bold">
								{formatNumber(items.reduce((sum, i) => sum + i.concreteVolume, 0))} {t("pricing.studies.units.m3")}
							</p>
						</div>
						<div>
							<span className="text-muted-foreground">{t("pricing.studies.summary.totalRebar")}:</span>
							<p className="font-bold">
								{formatNumber(items.reduce((sum, i) => sum + i.steelWeight, 0))} {t("pricing.studies.units.kg")}
							</p>
						</div>
					</div>
				</div>
