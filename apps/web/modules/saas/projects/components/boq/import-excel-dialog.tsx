	...
	return (
		...
		>
			<DialogContent className="sm:max-w-3xl p-0 gap-0 rounded-2xl overflow-hidden flex flex-col max-h-[90dvh]">
				...
				<div className="p-5 min-h-0 overflow-y-auto flex-1">
					...
					{step === 1 && (
						<div className="space-y-4">
							<div
								...
								className="border-2 border-dashed border-input rounded-xl p-12 flex flex-col items-center justify-center cursor-pointer hover:border-chart-4 transition-colors"
							>
								{isProcessing ? (
					...
					{step === 2 && (
						<div className="space-y-4">
							...
							{/* Mapping table */}
							<div className="border-2 rounded-xl overflow-hidden overflow-x-auto">
								<table className="w-full text-sm">
									...
									<tbody>
										{mappings.map((mapping, idx) => (
											<tr
												key={idx}
												className="border-b-2 last:border-0"
											>
							...
							{!hasRequiredMapped && (
								<div className="flex items-center gap-2 text-xs text-chart-1 bg-chart-1/15 p-3 rounded-xl">
					...
					{step === 3 && (
						<div className="space-y-4">
							...
							<div className="flex flex-wrap gap-2">
								...
								>
									<CheckCircle2 className="h-3.5 w-3.5 me-1 text-success" />
								...
								{skippedCount > 0 && (
									<Badge
										variant="outline"
										className="rounded-lg px-3 py-1 text-xs text-chart-1 border-chart-1/30"
									>
							...
							<div className="border-2 rounded-xl overflow-hidden overflow-x-auto">
								<table className="w-full text-sm">
									<thead>
										<tr className="border-b-2">
											<th className="text-start px-3 py-2 font-medium text-muted-foreground text-xs">
												#
											</th>
											<th className="text-start px-3 py-2 font-medium text-muted-foreground text-xs">
												{t("table.code")}
									...
									<tbody>
										{parsedItems.slice(0, 10).map((item, idx) => (
											...
											>
												...
												<td className="px-3 py-2 text-xs text-card-foreground max-w-[200px] truncate">
													{item.description}
												</td>
												<td className="px-3 py-2 text-xs text-muted-foreground">
													{item.unit}
				...
				{/* Footer */}
				<div className="bg-muted border-t-2 px-5 py-3 flex gap-3 justify-between">
					<div>
						{step > 1 && (
							<Button
								type="button"
