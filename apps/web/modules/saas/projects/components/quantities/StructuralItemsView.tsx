...
export function StructuralItemsView({
	organizationId,
	organizationSlug,
	projectId,
}: Props) {
	...
	// Flatten structural items from all phases + unassigned
	type StructuralItemWithPhase = {
		id: string;
		costStudyId: string;
		category: string;
	...
	const allItems: StructuralItemWithPhase[] = [];

	// Items from phases
	for (const phase of breakdownData?.phases ?? []) {
		for (const item of phase.structural) {
			allItems.push({
				id: item.id,
	...
	// Unassigned items
	for (const item of breakdownData?.unassigned?.structural ?? []) {
		allItems.push({
			id: item.id,
			costStudyId: item.costStudyId,
			category: item.category,
		...
		return (
			<div className="flex flex-col items-center justify-center py-20 text-center">
				<p className="text-sm text-slate-500 dark:text-slate-400">
					{t("projectQuantities.empty.noStructuralItems")}
				</p>
			</div>
		);
	}
					...
					<Table>
						<TableHeader>
							<TableRow>
								...
								<TableHead>{t("projectQuantities.table.subCategory")}</TableHead>
								<TableHead>{t("projectQuantities.table.description")}</TableHead>
								<TableHead className="text-end">
									{t("projectQuantities.table.quantity")}
								</TableHead>
								<TableHead>{t("projectQuantities.table.unit")}</TableHead>
								<TableHead className="text-end">
