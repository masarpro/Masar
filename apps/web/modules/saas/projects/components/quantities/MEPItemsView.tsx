...
export function MEPItemsView({
	...
	// Flatten MEP items from all phases + unassigned
	type MEPItemWithPhase = {
		id: string;
		costStudyId: string;
		category: string;
		name: string;
	...
	const allItems: MEPItemWithPhase[] = [];

	// Items from phases
	for (const phase of breakdownData?.phases ?? []) {
		for (const item of phase.mep) {
			allItems.push({
				id: item.id,
	...
	// Unassigned items
	for (const item of breakdownData?.unassigned?.mep ?? []) {
		allItems.push({
			id: item.id,
			costStudyId: item.costStudyId,
			category: item.category,
					...
					<Table>
						<TableHeader>
							<TableRow>
								...
								<TableHead>{t("projectQuantities.table.system")}</TableHead>
